...
# Load Cleaned Shapefiles with Boundary Length Modifier Data 
...
maxFWB <- problem(habitatProportion, c("fox", "water"), cost_column = "revenue") %>% 
  add_max_features_objective(cost) %>% 
  add_absolute_targets(c(foxTarget, waterTarget) * 1.32) %>% 
  add_boundary_penalties(penalty = 0.1, edge_factor = 0, 
                         data = bm)


#eval_cost_summary( maxFWB, solve(maxFWB) %>% dplyr::select(solution_1))
...
# 2015 Realized Retirement ("Baseline") Scenario
